...
class GraphClassifier(nn.Module):
    def __init__(self, hidden_dim: int, num_classes: int, pooling_op: str):
        ...
        self.graph_classifier = nn.Sequential(nn.Linear(hidden_dim,hidden_dim),
                                              nn.ReLU(),
                                              nn.Linear(hidden_dim,num_classes))# TODO: Define the graph classifier
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        classifier_logits=self.graph_classifier(x)
